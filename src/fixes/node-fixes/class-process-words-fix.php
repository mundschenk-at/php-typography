...
class Process_Words_Fix extends Abstract_Node_Fix {
	...
	public function apply( \DOMText $textnode, Settings $settings, $is_title ) {
		...
		$mixed_caps       = empty( $settings->hyphenate_all_caps ) ? Text_Parser::ALLOW_ALL_CAPS : Text_Parser::NO_ALL_CAPS;
		$letter_caps      = empty( $settings->hyphenate_all_caps ) ? Text_Parser::NO_ALL_CAPS : Text_Parser::ALLOW_ALL_CAPS;
		$mixed_compounds  = empty( $settings->hyphenate_compounds ) ? Text_Parser::ALLOW_COMPOUNDS : Text_Parser::NO_COMPOUNDS;
		$letter_compounds = empty( $settings->hyphenate_compounds ) ? Text_Parser::NO_COMPOUNDS : Text_Parser::ALLOW_COMPOUNDS;
		...
		$tokens[ Token_Fix::COMPOUND_WORDS ] = ! empty( $settings->hyphenate_compounds ) ? $text_parser->get_words( Text_Parser::NO_ALL_LETTERS, $letter_caps, Text_Parser::REQUIRE_COMPOUNDS ) : [];
